...
import { PERSONALITY_INFO } from '@/lib/mbti/scoring';
...
export default function ResultPage() {
  ...
  const personalityInfo = PERSONALITY_INFO[displayType];
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <button
                  onClick={() => {
                    if (navigator.share) {
                      navigator.share({
                        title: `I'm an ${displayType} - ${personalityInfo.name}`,
              ...
              <div className="mt-8 pt-6 border-t border-white/10">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="p-3 rounded-lg bg-white/5">
                    <div className="text-white/60 mb-1">Questions Answered</div>
                    <div className="text-2xl font-bold text-white">{result.answers.length}</div>
                  </div>
                  <div className="p-3 rounded-lg bg-white/5">
                    <div className="text-white/60 mb-1">Strongest Preference</div>
                    <div className="text-2xl font-bold text-white">
                      {getStrongestPreference(result)}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="w-full p-6 text-center space-y-4">
          ...
          <div className="inline-flex flex-col items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
            <p className="text-white/70 text-sm">Enjoyed this free assessment?</p>
            <a
          ...
          <p className="text-white/40 text-sm">
            This assessment is for self-discovery purposes. Results may vary.
          </p>
        </div>
      </div>
    </main>
...
function getStrongestPreference(result: { scores: { EI: { percentage: number }; SN: { percentage: number }; TF: { percentage: number }; JP: { percentage: number } } }): string {
  const dimensions = [
  ...
  const trait = percentage >= 50 ? strongest.labels[0] : strongest.labels[1];
